...
export default function StudentLeaderboard() {
  ...
  return (
    <div className="min-h-dvh bg-surface-container-lowest text-on-background flex flex-col">
      ...
      <main className="pt-20 pb-28 max-w-7xl mx-auto w-full flex-1 flex flex-col">
        ...
        {PLAYERS.length > 0 ? (
          <>
            ...
            <section className="px-6 py-8 flex items-end justify-center gap-4">
              <div className="flex flex-col items-center w-1/3 max-w-[120px] mb-4">
                <div className="relative mb-3">
                  ...
                  <div className="w-16 h-16 rounded-full border-2 border-outline-variant bg-surface-container-high flex items-center justify-center font-inter font-bold text-on-surface">{podium[1]?.name.split(' ').map(n => n[0]).join('')}</div>
                </div>
              ...
              <div className="flex flex-col items-center w-1/3 max-w-[140px] z-10">
                <div className="relative mb-4">
                  ...
                  <div className="w-24 h-24 rounded-full border-4 border-secondary-container shadow-[0_0_20px_rgba(195,244,0,0.2)] bg-surface-container-high flex items-center justify-center font-inter font-bold text-xl text-on-surface">{podium[0]?.name.split(' ').map(n => n[0]).join('')}</div>
                </div>
              ...
              <div className="flex flex-col items-center w-1/3 max-w-[120px] mb-2">
                <div className="relative mb-3">
                  ...
                  <div className="w-16 h-16 rounded-full border-2 border-outline-variant bg-surface-container-high flex items-center justify-center font-inter font-bold text-on-surface">{podium[2]?.name.split(' ').map(n => n[0]).join('')}</div>
                </div>
            ...
            <section className="px-6 flex flex-col gap-3 pb-6">
              {rest.map(p => (
                <div key={p.rank} className={`bg-surface-container rounded-xl p-4 flex items-center gap-4 border transition-colors relative overflow-hidden
                  ${p.isMe ? 'border-primary-container/50 bg-primary-container/5' : 'border-outline-variant/30 hover:bg-surface-container-high'}`}>
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-surface-bright" />
                  {p.isMe && <div className="absolute top-2 right-2 text-[10px] font-lexend font-bold text-primary uppercase tracking-wider">You</div>}
                  <div className="w-8 text-center text-headline-md font-inter font-bold text-on-surface-variant">{p.rank}</div>
                  <div className={`w-12 h-12 rounded-full border flex items-center justify-center font-inter font-bold text-sm shrink-0
                    ${p.isMe ? 'bg-primary-container/20 border-primary-container text-primary' : 'bg-surface-container-high border-outline-variant text-on-surface'}`}>
                    {p.name.split(' ').map(n => n[0]).join('')}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className={`text-label-lg font-lexend truncate ${p.isMe ? 'text-primary' : 'text-on-background'}`}>{p.name}</h4>
                    <div className="flex items-center gap-3 mt-1">
                      <span className="flex items-center gap-1 text-on-surface-variant text-label-sm font-lexend"><span className="material-symbols-outlined text-[14px]">check_circle</span>{p.attendance}</span>
                      <span className="flex items-center gap-1 text-on-surface-variant text-label-sm font-lexend"><span className="material-symbols-outlined text-[14px]">star</span>{p.stars}</span>
                    </div>
                  </div>
          ...
          <EmptyState
            ...
            description="The season has just started! No rankings are available yet. Keep practicing to see your name here."
          />
        )}
      </main>
      <BottomNav />
