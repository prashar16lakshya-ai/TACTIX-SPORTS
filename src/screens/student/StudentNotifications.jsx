    ...
    <div className="min-h-dvh bg-[#0A0E1A] text-on-background flex flex-col">
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto pt-20 pb-28 px-6 flex flex-col gap-6">
        ...
        <div className="flex gap-2 overflow-x-auto hide-scrollbar">
          {['All Alerts', 'Team Updates', 'Personal Goals'].map((f, i) => (
            <button key={f} className={`px-6 py-2.5 rounded-full font-lexend text-label-lg whitespace-nowrap min-h-[48px] transition-colors
              ${i === 0 ? 'bg-primary-container text-on-primary-container shadow-[0_4px_12px_rgba(0,82,255,0.2)]' : 'bg-[#121826] text-on-surface-variant border border-[#1E2638] hover:bg-surface-variant/50'}`}>{f}</button>
          ))}
        </div>
        <div className="flex flex-col gap-4">
          {NOTIFS.length > 0 ? NOTIFS.map((n, i) => (
            <div key={i} className={`relative bg-[#121826] p-6 rounded-xl border border-[#1E2638] flex gap-5 items-start hover:border-outline-variant transition-colors cursor-pointer shadow-lg shadow-black/20 ${!n.unread ? 'opacity-70 hover:opacity-100' : ''}`}>
              {n.unread && <div className="absolute top-4 right-4 w-3 h-3 rounded-full bg-secondary-container shadow-[0_0_12px_rgba(21,81,47,0.6)]" />}
